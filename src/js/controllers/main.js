...
function formUpdateCtrl($scope, $http, $window) {
  ...
  var bac = { header: { 'Auth': "hjk" } };

  $scope.saveFormValues = function () {
    this.route = $scope.routeUrl + '/file/updateFormData';
    $http.post(this.route, $scope.customers, bac).then(function (response) {
      $scope.customers = response.data;
    });
  }
...
updateDataCtrl.$inject = ['$scope', '$http'];
